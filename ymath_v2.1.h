...
array LG_grad(array a);						//19.线性回归:  梯度下降法，取固定步长
array LDA(array a);							//20.线性判别:  返回样本矩阵的线性判别矩阵
...
array LG_grad(array a)
{
	array x = createarray(a.x, a.y);
	evaluation(x, a);
	array w = createarray(a.y, 1);
	array y = createarray(a.x, 1);
	array grad = createarray(a.y, 1);
	addline(x, 0, x.y);
	for (int i = 1; i <= x.x; i++)
		x.p[i][x.y - 1] = 1;
	for (int i = 1; i <= x.x; i++)
	...
	{
		grad = add(dot(dot(transpose(x), x), w), dot(transpose(x), y), -1);
		w = add(w, grad, -0.001);
	}
	return w;
}
