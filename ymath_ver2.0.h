...
bool evaluation(array a, array b);			//赋值:		把b矩阵的元素赋给a  成功返回1，大小不一返回0
bool output(array a);						//输出
array transpose(array &a);					//转置:		原矩阵被转置，返回转置矩阵
array deleteline(array &a, int x, int y);   //删除行列:	删除a的x行或y列，取值超出范围则空操作，注意搬移后多出来的内存被释放掉了  请勿往这里赋值
array addline(array &a, int x, int y);		//增加行列:	增加第x行或第y列,元素为0，取值0则空操作
array deletearray(array &a);                //释放矩阵
array cofactor(array a, int x, int y);      //求余子式:	求a[x][y]的余子式,x or y超出范围返回空矩阵,不会操作原矩阵
float mod(array a);							//求模:		不是方阵返回0
array dot(array a, array b);				//矩阵点积:	若行列冲突，返回行列数为0的空矩阵
array moveline(array &a,int x,int y,float c);//行操作:	原矩阵变为x+y*c，返回操作后的矩阵		
array triangulation(array a);				//上三角化:	原矩阵不动，返回上三角阵
int rank(array a);							//求秩
array inverse(array a);						//求逆，不会操作原矩阵，不可逆则返回空矩阵
